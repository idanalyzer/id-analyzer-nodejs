...
class CoreAPI {
    ...
    scan(options){
        ...
        return new Promise(function (resolve, reject) {
            ...
            axios.post(apiURL, payload, {timeout: 60000, headers: { 'content-type': 'application/json' }}).then(function (response) {
...
class DocuPass {
    ...
    _create(docupass_module){
        ...
        return new Promise(function (resolve, reject) {
            axios.post(apiURL, payload, {timeout: 15000, headers: { 'content-type': 'application/json' }}).then(function (response) {
                resolve(response.data);
            }).catch(function (error) {
                if (error.response) {
                    reject(new Error('Connecting to API Server failed: HTTP '+  response.status));
...
class Vault {
    ...
    list(options)
    {
        let payload = {}
        if(options.filter){
            if(!Array.isArray(options.filter) || options.filter.length<1 || options.filter.length>5){
                throw new Error("Filter must be an array with max 5 filter statements.");
            }
